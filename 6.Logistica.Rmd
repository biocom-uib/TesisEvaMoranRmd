...
## Tablas para las regresiones logísticas
...
### Influencia de características previas en la COVID
...
* Valores VIF: todos entre `r round(min(vif(mod.COVID)[,1]),4)` y `r round(max(vif(mod.COVID)[,1]),4)`. 

#### COVID según gravedad
...
* ORa: La odds ratio ajustada
$$
\frac{\text{Odds(Nivel de interés|Variable sí)}/\text{Odds(Nivel basal|Variable sí)}}{\text{Odds(Nivel de interés|Variable no)}/\text{Odds(Nivel basal|Variable no)}}
$$
* IC 95%: Intervalo de confianza del 95% para la ORa
...
#### COVID asintomática, leve o grave contra No COVID
...
#### COVID ante o periparto contra No COVID
...
#### COVID anteparto asintomática, leve o grave  contra No COVID
...
#### COVID periparto asintomática, leve o grave  contra No COVID
...
### Preeclampsia grave en preeclámpsicas
...
#### COVID No, asintomática, leve o grave
...
#### COVID ante o periparto contra No
...
### Tipo de HPP en gestantes con HPP
...
```{r}
mod.COVID=glm(UCIN~COVID+Pesos+Prematuro+AnCon+RetrasoCF+GM+APGAR,data=DFL.niños,family=binomial(link="logit"))
DFOR=or_model_summary(round_p=8,model = mod.COVID)
...
```{r}
mod.COVID=glm(UCIN~Sint+Pesos+Prematuro+AnCon+RetrasoCF+GM+APGAR,data=DFL.niños,family=binomial(link="logit"))
DFOR=or_model_summary(round_p=8,model = mod.COVID)
...
```{r}
mod.COVID=glm(UCIN~PreP+Pesos+Prematuro+AnCon+RetrasoCF+GM+APGAR,data=DFL.niños,family=binomial(link="logit"))
DFOR=or_model_summary(round_p=8,model = mod.COVID)
...
```{r}
mod.COVID=glm(UCIN~SintPre+Pesos+Prematuro+AnCon+RetrasoCF+GM+APGAR,data=DFL.niños,family=binomial(link="logit"))
DFOR=or_model_summary(round_p=8,model = mod.COVID)
...
```{r}
mod.COVID=glm(UCIN~SintPeri+Pesos+Prematuro+AnCon+RetrasoCF+GM+APGAR,data=DFL.niños,family=binomial(link="logit"))
DFOR=or_model_summary(round_p=8,model = mod.COVID)
...
```{r}
mod.COVID=glm(UCIN~SegundOla+Pesos+Prematuro+AnCon+RetrasoCF+GM+APGAR,data=DFL.niños,family=binomial(link="logit"))
DFOR=or_model_summary(round_p=8,model = mod.COVID)
...
```{r}
mod.COVID=glm(UCIN~SegundOla+Pesos+Prematuro+AnCon+RetrasoCF+GM+APGAR,data=DFL.niños[DFL.niños$COVID==1,],family=binomial(link="logit"))
DFOR=or_model_summary(round_p=8,model = mod.COVID)
