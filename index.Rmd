...
El análisis tanto descriptivo como inferencial de los datos se ha llevado a cabo usando R versión 4.2.2. Para los gráficos se ha usado el paquete `ggplot2` de R. Este documento se ha escrito en *R Markdown*, usando *RStudio* como editor de textos y el paquete `bookdown` para compilar los ficheros *markdown* y producir el documento html final. Los ficheros *markdown* se encuentran en el repositorio https://github.com/biocom-uib/TesisEvaMoranRmd.
...
**Variables categóricas**: Se ha usado sistemáticamente el test exacto de Fisher para la comparación bilateral de pares de proporciones o de odds ratios, según correspondiera a la naturaleza del contraste. Como el tamaño de las muestras siempre lo ha permitido, Los intervalos de confianza para RA se han calculado con la función `prop.test` con ajuste de continuidad, y los intervalos para RR se han calculado con la función `ReslRisk`del paquete `DescTools`. La comparación de pares de diferencias de proporciones se ha realizado por el método de marcas de Martin-Herranz-Álvarez; mientras que en la comparación de pares de cocientes de proporciones  (pares de RR) o de pares de OR se han usado los tests usuales basados en aproximaciones lognormales. Los tres métodos se han implementado en funciones de R. 


La igualdad de distribuciones de variables categóricas politómicas se ha contrastado sistemáticamente mediante tests ji cuadrado de bondad de ajuste, usando el método de Montecarlo (basado en 10000 simulaciones) cuando no se cumplían las condiciones teóricas para poder usar su versión paramétrica. 

El análisis multivariante de variables categóricas y el cálculo de odds ratios ajustadas se ha llevado a cabo mediante regresiones logísticas.
...
#
Casos2w$SINTOMAS_DIAGNOSTICO=factor(Casos2w$SINTOMAS_DIAGNOSTICO,levels=c("Asintomatica", "Sintomas leves",       "Neumonia"),ordered=TRUE)
Casos2w$SINTOMAS_DIAGNOSTICO=plyr::revalue(Casos2w$SINTOMAS_DIAGNOSTICO, c("Asintomatica"="Asintomática", "Sintomas leves"="Síntomas leves","Neumonia"="Neumonía"))
Casos2wGM$SINTOMAS_DIAGNOSTICO=factor(Casos2wGM$SINTOMAS_DIAGNOSTICO,levels=c("Asintomatica", "Sintomas leves",       "Neumonia"),ordered=TRUE)
Casos2wGM$SINTOMAS_DIAGNOSTICO=plyr::revalue(Casos2wGM$SINTOMAS_DIAGNOSTICO, c("Asintomatica"="Asintomática", "Sintomas leves"="Síntomas leves","Neumonia"="Neumonía"))
#
...
CasosGM$SINTOMAS_DIAGNOSTICO=factor(CasosGM$SINTOMAS_DIAGNOSTICO,levels=c("Asintomatica", "Sintomas leves",       "Neumonia"),ordered=TRUE)
CasosGM$SINTOMAS_DIAGNOSTICO=plyr::revalue(CasosGM$SINTOMAS_DIAGNOSTICO, c("Asintomatica"="Asintomática", "Sintomas leves"="Síntomas leves","Neumonia"="Neumonía"))
Sint=Casos$SINTOMAS_DIAGNOSTICO
#
Casos1w$SINTOMAS_DIAGNOSTICO=factor(Casos1w$SINTOMAS_DIAGNOSTICO,levels=c("Asintomatica", "Sintomas leves",       "Neumonia"),ordered=TRUE)
Casos1w$SINTOMAS_DIAGNOSTICO=plyr::revalue(Casos1w$SINTOMAS_DIAGNOSTICO, c("Asintomatica"="Asintomática", "Sintomas leves"="Síntomas leves","Neumonia"="Neumonía"))
Casos1wGM$SINTOMAS_DIAGNOSTICO=factor(Casos1wGM$SINTOMAS_DIAGNOSTICO,levels=c("Asintomatica", "Sintomas leves",       "Neumonia"),ordered=TRUE)
Casos1wGM$SINTOMAS_DIAGNOSTICO=plyr::revalue(Casos1wGM$SINTOMAS_DIAGNOSTICO, c("Asintomatica"="Asintomática", "Sintomas leves"="Síntomas leves","Neumonia"="Neumonía"))
#
n_I=dim(Casos)[1]
n_NI=dim(Controls)[1]
n_I1=dim(Casos1w)[1]
...
Defino para Casos y Controles la variable **CesAnt** (¿césarea anterior? 0 no, 1 sí) buscando las cesáreas previas en las variables "Indicación de cesárea" y "Especificar otra indicación". 
...
Defino para Casos y Controles la variable **Nalgas** (0 de cara, 1 de pies, 2 transversal/oblícua, NA malposiciones no especificadas) basándome en las variables "Indicación de cesárea", "Especificar otra indicación" y "Especificar otra indicación parto instrumental".
...
Defino la variable **Robson** para Casos y Controles
...
# Comparación entre olas: Casos y controles
...
# Tabla comparación Anteparto-Periparto
