...
Diferencia en la distribución de grupos Robson en las dos olas: Test ji cuadrado de Montecarlo (hay muy pocas gestantes del grupo 9), p-valor `r round(CT,3)`. No hay evidencia de diferencia en la distribución de los grupos en las dos olas.
...
Diferencia en la distribución de grupos Robson entre casos y controles: Test ji cuadrado de Montecarlo (hay muy pocas gestantes del grupo 9), p-valor `r round(CT,5)`. 
...
pNeum=pTRCaS[,3])
row.names(DFTRCaS)=c()
names(DFTRCaS)=c("Grupo Robson", "Total infectadas", "Asintomáticas", "%", "Síntomas leves", "%", "Neumonía", "%")
...
```{r ,fig.cap="\\s"}
Tipo=factor(rep(c("Asintomáticas","Síntomas leves", "Neumonía"), each=10))
...
En la tabla que sigue, para cada grupo Robson, el p-valor es del contraste ji cuadrado de si la distribución de los síntomas de las infectadas de ese grupo es igual o diferente de la distribución de los síntomas de las infectadas. Como ves, las distribuciones de síntomas en los grupos 1 y 10 son significativamente diferentes del global.
...
names(DFTRCaWS)=c("Grupo Robson", "Total infectadas 1a ola", "Asintomáticas 1a ola", "%", "Síntomas leves 1a ola", "%", "Neumonía 1a ola", "%","Total infectadas 2a ola", "Asintomáticas 2a ola", "%", "Síntomas leves 2a ola", "%", "Neumonía 2a ola", "%")
...
```{r ,fig.cap="\\s"}
Tipo=ordered(rep(c("Asintomáticas","Síntomas leves", "Neumonía"), each=10),levels=c("Asintomáticas","Síntomas leves", "Neumonía"))
Grupo=factor(rep(1:10 , 3))
...
Tipo=ordered(rep(c("Asintomáticas","Síntomas leves", "Neumonía"), 20),levels=c("Asintomáticas","Síntomas leves", "Neumonía"))
...
En la tabla que sigue, para cada grupo y cada ola, el p-valor es del contraste ji cuadrado de si la distribución de los síntomas de las infectadas de ese grupo y esa ola es igual o diferente de la distribución de los síntomas de las infectadas de ese grupo en el total de las dos olas.
...
names(DFTRCaWSC)=c("Grupo Robson", "Asintomáticas 1a ola", "% cesáreas en asintomáticas 1a ola", "Síntomas leves 1a ola", "% cesáreas en síntomas leves 1a ola", "Neumonías 1a ola", "% cesáreas en neumonía 1a ola",  "Asintomáticas 2a ola","% cesáreas en asintomáticas 2a ola", "Síntomas leves 2a ola", "% cesáreas en síntomas leves 2a ola", "Neumonías 2a ola", "% cesáreas en neumonía 2a ola")
...
pp[i]=prop.trend.test(EEw[1,],colSums(EEw),score=prop.table(EE,margin=2)[1,])$p.value
}
...
names(tabla)=c("Grupo Robson", "OR asintomáticas", "p-valor asintomáticas","OR síntomas leves","p-valor síntomas leves","OR neumonía","p-valor neumonía")
row.names(tabla)=c()
...
rowSums(TCaNeu),TCaNeu[,1],round(100*prop.table(TCaNeu,margin=1)[,1],2))
      names(dades)=c("Grupo Robson", "Inf. asintomáticas", "Cesáreas", "Porcentaje", 
                     "Inf. síntomas leves", "Cesáreas", "Porcentaje",
                     "Inf. Neumonía", "Cesáreas", "Porcentaje")
dades %>%
  kbl() %>%
  kable_styling()%>%     
  scroll_box(width="100%", box_css="border: 0px;")
...
pp[1]=prop.trend.test(EE[,2],EE[,1],score=(EE[,2]+EE[,4])/(EE[,1]+EE[,3]))$p.value
pp[2]=prop.trend.test(EE[,2],EE[,1],score=(EE[,2]+EE[,6])/(EE[,1]+EE[,5]))$p.value
pp[3]=prop.trend.test(EE[,4],EE[,3],score=(EE[,4]+EE[,6])/(EE[,3]+EE[,5]))$p.value
```

* Asintomáticas contra síntomas leves: $`r signif(pp[1],1)`$
* Asintomáticas contra neumonía: $`r signif(pp[2],1)`$
* Síntomas leves contra neumonía: $`r signif(pp[3],1)`$


Qué pasa si agrupamos asintomáticas+síntomas leves contra neumonía? Hay diferencia en la 10
...
dt=data.frame(1:10,colSums(EE1),EE1[1,],round(100*prop.table(EE1,margin=2)[1,],2),colSums(EE2),EE2[1,],round(100*prop.table(EE2,margin=2)[1,],2),round(OR,2),round(pp,3))
names(dt)=c("Grupo Robson", "Inf. asintomáticas o leves", "Cesáreas", "Porcentaje",  "Inf. Neumonía", "Cesáreas", "Porcentaje","OR","p-valor test de Fisher")
dt %>%
  kbl() %>%
  kable_styling()%>%     
  scroll_box(width="100%", box_css="border: 0px;")
...
#
CasosPeriLeves=Casos
CasosPeriLeves=CasosPeriLeves[CasosPeriLeves$Sint==1|CasosPeriLeves$Sint==2,]
CasosPeriLeves=CasosPeriLeves[CasosPeriLeves$PreP!="Anteparto",]
CasosPeriLeves=droplevels(CasosPeriLeves)
...
TRCaPL=table(CasosPeriLeves$Robson,CasosPeriLeves$Cesárea) 
...
ggplot(data, aes(fill=Tipo, y=valor, x=Grupo)) + 
  ...
  ylab("")+
  xlab("")+
  ggtitle("Cesáreas en grupos Robson de infectadas periparto asintomáticas o leves")+
  scale_fill_brewer(palette = "Pastel1") 
```
...
```{r}
TablaRobsonGlobal(TRCaPL,TRCo,"Peripartos asintomáticos o leves","")
```
...
```{r}
TablaRobsonGlobal(TRCaPG,TRCo,"Peripartos neumonías","")
```

Qué grupos tienen proporciones de cesáreas en infectadas periparto no graves o neumonias significativamente diferentes? Bastantes!
...
dt=data.frame(1:10,colSums(EE1),EE1[1,],round(100*prop.table(EE1,margin=2)[1,],2),colSums(EE2),EE2[1,],round(100*prop.table(EE2,margin=2)[1,],2),round(OR,2),round(pp,3))
names(dt)=c("Grupo Robson", "Periparto neumonía", "Cesáreas", "Porcentaje",  "Periparto no neumonía", "Cesáreas", "Porcentaje","OR","p-valor test de Fisher")
dt %>%
  kbl() %>%
  kable_styling()%>%     
  scroll_box(width="100%", box_css="border: 0px;")
...
pTRCa2wS[,3])
row.names(DFTRCaWS)=c()
names(DFTRCaWS)=c("Grupo Robson", "Total infectadas anteparto", "Asintomáticas prepartp", "%", "Síntomas leves anteparto", "%", "Neumonía anteparto", "%","Total infectadas periparto", "Asintomáticas periparto", "%", "Síntomas leves periparto", "%", "Neumonía peripartoa", "%")
...
En la tabla que sigue, para cada grupo y cada momento de diagnóatico, el p-valor es del contraste ji cuadrado de si la distribución de los síntomas de las infectadas de ese grupo y ese momento es igual o diferente de la distribución de los síntomas de las infectadas de ese grupo en el total de las dos olas.
...
names(DFTRCaWSC)=c("Grupo Robson", "Asintomáticas anteparto", "% cesáreas en asintomáticas anteparto", "Síntomas leves anteparto", "% cesáreas en síntomas leves anteparto", "Neumonías anteparto", "% cesáreas en neumonía anteparto",  "Asintomáticas periparto","% cesáreas en asintomáticas periparto", "Síntomas leves periparto", "% cesáreas en síntomas leves periparto", "Neumonías periparto", "% cesáreas en neumonía periparto")
