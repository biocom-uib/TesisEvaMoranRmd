...
colnames(EEExt)=c("Árabe", "Asiática", "Blanca", "Latinoamericana","Negra","Pérdidas")
...
pp=p.adjust(pp,method="bonferroni")
dt=data.frame(franja=c("Árabe", "Asiática", "Blanca", "Latinoamericana","Negra"),pp,OR,CI1,CI2)
names(dt)=c("Franja","p-valor ajustado","OR","Extremo inferior de IC", "Extremo superior de IC")
dt %>%
  kbl() %>%
  kable_styling()
...
Tabla.DMG1(I,NI,"Nulíparas", "Multíparas")
```
...
rownames(EEExt)=c("Árabe", "Asiática", "Blanca", "Latinoamericana","Negra")
colnames(EEExt)=Columnes.Sint
...
ppTR=p.adjust(ppTR,method="bonferroni")
dt=data.frame(franja=c("Árabe", "Asiática", "Blanca", "Latinoamericana","Negra"),ppTR)
...
* Asociación entre los grupos de sintomatología y EPC: test $\chi^2$ de Montecarlo, p-valor `r round(chisq.test(taula,simulate.p.value=TRUE,B=10000)$p.value,3)`
...
I=Casos$NULIPARA
taula=table(data.frame(I,Sint))[c(2,1) ,]
Tabla.DGSW(I,"Multípara", "Nulípara")
Barplot.DGSW(I, "Nulípara","Multípara")
```
...
* Asociación entre los grupos de sintomatología y muerte fetal: test $\chi^2$, p-valor `r round(chisq.test(taula,simulate.p.value=TRUE,B=10000)$p.value,3)`
